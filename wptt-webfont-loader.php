...
if ( ! class_exists( 'Mehndi_Tattoo_Artist_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Mehndi_Tattoo_Artist_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local mehndi_tattoo_artist_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $mehndi_tattoo_artist_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $mehndi_tattoo_artist_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$mehndi_tattoo_artist_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$mehndi_tattoo_artist_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$mehndi_tattoo_artist_font_family = trim( str_replace( array( "'", ';' ), '', $mehndi_tattoo_artist_font_family ) );
					$mehndi_tattoo_artist_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $mehndi_tattoo_artist_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $mehndi_tattoo_artist_font_family ] ) ) {
					$result[ $mehndi_tattoo_artist_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$mehndi_tattoo_artist_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$mehndi_tattoo_artist_font_family_url = $this->get_absolute_path( $mehndi_tattoo_artist_font_family_url );

					$result[ $mehndi_tattoo_artist_font_family ][] = $mehndi_tattoo_artist_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $mehndi_tattoo_artist_font_family ] = array_flip( array_flip( $result[ $mehndi_tattoo_artist_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Mehndi_Tattoo_Artist_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'wptt_get_webmehndi_tattoo_artist_font_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_web_font_url( $url, $format = 'woff2' ) {
		$font = new Mehndi_Tattoo_Artist_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
